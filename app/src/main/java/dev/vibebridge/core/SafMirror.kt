...
object SafMirror {
    ...
    fun release(ctx: Context, uri: Uri) {
        try {
            ...
            // permission already released or never granted
        }
    }

    fun mirror(ctx: Context, uri: Uri, ws: Workspace): VbResult<Int> = try {
        ...
        for (path in ws.tree()) {
            ...
            for (i in 0 until parts.size - 1) {
                dir = dir.findFile(parts[i])
                    ...
                    ?: return VbResult.Err("Cannot create folder ${parts[i]}.")
            }
            val name = parts.last()
            val target = dir.findFile(name)
                ?: dir.createFile("application/octet-stream", name)
                ?: return VbResult.Err("Cannot create file $name.")
