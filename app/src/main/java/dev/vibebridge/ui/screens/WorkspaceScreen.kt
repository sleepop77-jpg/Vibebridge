    ...
    Column(
        ...
        Stagger(0) {
            Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                Text("FILES", style = MaterialTheme.typography.displaySmall, color = AmberHi)
        ...
        Stagger(1) {
            VbPanel(title = "SANDBOX STORAGE") {
                ...
                ui.zipMsg?.let { Spacer(Modifier.height(8.dp)); VbBanner(kind = VbBannerKind.INFO, text = it) }
            }
        }
        Stagger(2) {
            VbPanel(title = "SAF MIRROR") {
                ...
                ui.mirrorMsg?.let { Spacer(Modifier.height(8.dp)); VbBanner(kind = VbBannerKind.INFO, text = it) }
            }
        }
        Stagger(3) {
