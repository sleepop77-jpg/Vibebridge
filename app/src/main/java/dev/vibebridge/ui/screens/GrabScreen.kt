...
fun GrabScreen(vm: GrabViewModel, gotoPush: () -> Unit) {
    ...
    Column(
        ...
        Stagger(0) { Text("GRAB", style = MaterialTheme.typography.displaySmall, color = AmberHi) }
        if (vm.strict) VbBanner(kind = VbBannerKind.WARN, text = "Strict mode is on. Payloads without the sentinel are rejected.")
        ...
        ui.parsed?.let { parsed ->
            Stagger(4) {
                VbPanel(title = "PLAN") {
                    parsed.warnings.forEach { w -> VbBanner(kind = VbBannerKind.WARN, text = w); Spacer(Modifier.height(6.dp)) }
                    ui.plan.forEach { rep ->
        ...
        ui.applied?.let { rep ->
            Stagger(5) {
                VbPanel(title = "APPLIED TO SANDBOX") {
                    ...
                    rep.errors.forEach { e -> Spacer(Modifier.height(6.dp)); VbBanner(kind = VbBannerKind.ERROR, text = e) }
                }
            }
        }
        Stagger(6) {
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                VbButtonSecondary(text = "CONTINUE TO PUSH", onClick = gotoPush, modifier = Modifier.weight(1f), enabled = ui.parsed != null && ui.parsed!!.ops.isNotEmpty(), disabledReason = "No operations staged.")
            }
        }
    }
}
